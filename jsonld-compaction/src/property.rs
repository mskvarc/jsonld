use crate::{
	CompactFragment, CompactIndexedFragment, Error, Options, add_value, compact_collection_with,
	compact_iri, compact_iri_with, compact_key, value_value,
};
	...
	if container.contains(ContainerKind::Graph) && container.contains(ContainerKind::Id) {
			...
			.is_none()
		{
			nest_result.insert(
				...
				jsonld_syntax::Object::default().into(),
			);
		}
