...
use crate::algorithm::iri::{Action, MalformedIri};
...
use jsonld_syntax::{
	self as syntax, CompactIri, ContainerKind, ExpandableRef, Keyword, LenientLangTag, Nullable,
	context::{
...
pub fn expand_iri_with_sync<'a, N, L, W>(
	...
	options: Options,
) -> Result<Option<Term<N::Iri, N::BlankId>>, Error>
where
	N: VocabularyMut,
	N::Iri: Clone + Eq + Hash,
		...
		Nullable::Some(ExpandableRef::String(value)) => {
			...
			if document_relative {
				if let Ok(iri_ref) = IriRef::parse(value) {
					if let Some(iri) =
						resolve_iri(env.vocabulary, iri_ref, active_context.base_iri())
					{
...
fn invalid_iri<N, L, W: jsonld_core::warning::Handler<N, Warning>>(
					...
					if let Some(container_value) = value.container {
						if options.processing_mode == ProcessingMode::JsonLd1_0 {
							match container_value {
								Nullable::Null
								| Nullable::Some(
									syntax::Container::Many(_)
									| syntax::Container::One(
										ContainerKind::Graph
										| ContainerKind::Id
										| ContainerKind::Type,
									),
...
pub(crate) fn process_context_sync<'l: 'a, 'a, N, L, W>(
	...
	base_url: Option<N::Iri>,
	mut options: Options,
) -> Result<crate::Processed<'l, N::Iri, N::BlankId>, Error>
where
	N: VocabularyMut,
	N::Iri: Clone + Eq + Hash,
	N::BlankId: Clone + PartialEq,
