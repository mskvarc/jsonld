...
use rdf_rs::{BlankId, BlankIdBuf, InvalidBlankId, LocalGenerator, LocalTerm, Term as RdfTerm};
...
/// it into `vocabulary`.
pub fn generator_next_id<V, G>(
	vocabulary: &mut V,
	generator: &mut G,
) -> ValidId<V::Iri, V::BlankId>
where
	V: VocabularyMut,
	G: LocalGenerator,
{
...
pub trait IdentifyAll<T, B> {
	fn identify_all_with<N: Vocabulary<Iri = T, BlankId = B>, G: LocalGenerator>(
		&mut self,
		vocabulary: &mut N,
		generator: &mut G,
	) where
