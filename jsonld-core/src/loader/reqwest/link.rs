...
mod tests {
    ...
    #[test]
    fn parse_link_1() {
        let link = Link::new(&HeaderValue::from_str("<http://www.example.org/context>; rel=\"context\"; type=\"application/ld+json\"").unwrap()).unwrap();
    ...
    fn parse_link_2() {
        let link =
            Link::new(&HeaderValue::from_str("<http://www.example.org/context>; rel=\"context\"; type=\"application/ld+json\"; foo=\"bar\"").unwrap()).unwrap();
        assert_eq!(link.href(), "http://www.example.org/context");
        assert_eq!(link.rel(), Some(b"context".as_slice()));
        assert_eq!(link.type_(), Some(b"application/ld+json".as_slice()))
    }
    ...
    #[test]
    fn parse_link_3() {
        let link = Link::new(&HeaderValue::from_str("<http://www.example.org/context>").unwrap()).unwrap();
        assert_eq!(link.href(), "http://www.example.org/context")
    }
}
