    ...
    for<'a> &'a Self: IntoIterator<Item = &'a Self::Item>,
{
    ...
    fn unordered_mapped_eq<'a, 'b, F: Clone + Fn(&'a <Self::Item as MappedEq>::BlankId) -> &'b <Self::Item as MappedEq>::BlankId>(
        &'a self,
        other: &Self,
        f: F,
    ) -> bool
...
impl<T: MappedEq> MappedEq for [T] {
    ...
    where
        Self::BlankId: 'a + 'b,
    {
        self.len() == other.len() && self.iter().zip(other).all(move |(a, b)| a.mapped_eq(b, f.clone()))
    }
}
...
fn opt_mapped_eq<'a, 'b, A: MappedEq, F: Clone + Fn(&'a A::BlankId) -> &'b A::BlankId>(a: Option<&'a A>, b: Option<&A>, f: F) -> bool
where
    A::BlankId: 'a + 'b,
{
    match (a, b) {
...
impl<T: Eq + Hash, B: Eq + Hash> MappedEq for super::Node<T, B> {
        ...
        B: 'a + 'b,
    {
        opt_mapped_eq(self.id.as_ref(), other.id.as_ref(), f.clone())
            ...
            && opt_mapped_eq(self.reverse_properties_entry(), other.reverse_properties_entry(), f)
    }
}
