...
pub struct ReverseProperties<T = IriBuf, B = BlankIdBuf>(IndexMap<Id<T, B>, ReversePropertyNodes<T, B>, DefaultHashBuilder>);
...
impl<T: Eq + Hash, B: Eq + Hash> ReverseProperties<T, B> {
    ...
    #[inline(always)]
    pub fn get<'a, Q: ?Sized + Hash + indexmap::Equivalent<Id<T, B>>>(&self, prop: &Q) -> Nodes<'_, T, B>
    where
        T: 'a,
    {
...
impl<T: Eq + Hash, B: Eq + Hash> Extend<(Id<T, B>, Vec<IndexedNode<T, B>>)> for ReverseProperties<T, B> {
...
impl<'a, T, B> Iterator for Iter<'a, T, B> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.inner.next().map(|(property, objects)| (property, objects.as_slice()))
    }
}
