use crate::{ExpandedDocument, FlattenedDocument, Id, Indexed, IndexedNode, IndexedObject, Node, Object, flattening::NodeMap, object};
...
enum QuadsFrame<'a, T, B> {
    ...
    NodeProperties(Option<&'a Id<T, B>>, &'a Id<T, B>, object::node::properties::Iter<'a, T, B>),
    NodeReverseProperties(Option<&'a Id<T, B>>, &'a Node<T, B>, object::node::reverse_properties::Iter<'a, T, B>),
    NodePropertyObjects(Option<&'a Id<T, B>>, &'a Id<T, B>, &'a Id<T, B>, std::slice::Iter<'a, IndexedObject<T, B>>),
    NodeReversePropertySubjects(Option<&'a Id<T, B>>, &'a Node<T, B>, &'a Id<T, B>, std::slice::Iter<'a, IndexedNode<T, B>>),
}
...
impl<'a, T, B> Quads<'a, T, B> {
    fn push_object(&mut self, graph: Option<&'a Id<T, B>>, object: &'a Indexed<Object<T, B>>) {
        match object.inner() {
            ...
            Object::List(objects) => self.stack.push(QuadsFrame::IndexedObjectSlice(graph, objects.iter())),
            Object::Value(_) => (),
        }
    }
    ...
    fn push_node(&mut self, graph: Option<&'a Id<T, B>>, node: &'a Node<T, B>) {
        if let Some(id) = &node.id {
            ...
            if let Some(reverse_properties) = node.reverse_properties_entry() {
                self.stack.push(QuadsFrame::NodeReverseProperties(graph, node, reverse_properties.iter()));
            }

            self.stack.push(QuadsFrame::NodeProperties(graph, id, node.properties().iter()));

            if let Some(types) = &node.types {
                self.stack.push(QuadsFrame::NodeTypes(graph, id, types.iter()));
            }
        }
    }
}
        ...
        while let Some(last) = self.stack.last_mut() {
            match last {
                ...
                QuadsFrame::NodeTypes(graph, subject, types) => {
                    ...
                    match types.next() {
                        Some(ty) => return Some(QuadRef(graph, subject, PropertyRef::Type, ObjectRef::<T, B>::Ref(ty))),
                        None => {
                            self.stack.pop();
                        }
                    }
                ...
                QuadsFrame::NodeProperties(graph, subject, properties) => {
                    ...
                    match properties.next() {
                        Some((property, objects)) => self.stack.push(QuadsFrame::NodePropertyObjects(graph, subject, property, objects.iter())),
                        None => {
                            self.stack.pop();
                        }
                    }
                ...
                QuadsFrame::NodeReverseProperties(graph, object, reverse_properties) => {
                    ...
                    match reverse_properties.next() {
                        Some((property, subjects)) => self
                            .stack
                            .push(QuadsFrame::NodeReversePropertySubjects(graph, object, property, subjects.iter())),
                        None => {
                            self.stack.pop();
                        }
                    }
