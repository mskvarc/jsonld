use super::{
	RDF_DIRECTION, RDF_FIRST, RDF_JSON, RDF_NIL, RDF_REST, RDF_VALUE, RdfDirection, XSD_BOOLEAN,
...
use rdf_rs::vocabulary::{IriVocabularyMut, Vocabulary, VocabularyMut};
use rdf_rs::{Literal, LocalGenerator};
...
fn static_datatype(iri: iri_rs::Iri<&'static str>) -> rdf_rs::Datatype {
	rdf_rs::Datatype::new(IriBuf::from(iri))
		.unwrap_or_else(|e| unsafe { rdf_rs::Datatype::new_unchecked(e.into_iri()) })
}

/// Build a [`rdf_rs::Datatype`] from a vocabulary handle.
fn datatype_of<V: rdf_rs::vocabulary::IriVocabulary>(
	vocabulary: &V,
	iri: &V::Iri,
) -> rdf_rs::Datatype {
	let buf = IriBuf::from(vocabulary.iri(iri).unwrap());
	rdf_rs::Datatype::new(buf)
		.unwrap_or_else(|e| unsafe { rdf_rs::Datatype::new_unchecked(e.into_iri()) })
}
		...
		match self {
				...
				match direction {
					Some(direction) => match rdf_direction {
						Some(RdfDirection::I18nDatatype) => {
							let ty = rdf_rs::Datatype::new(i18n(language, *direction))
								.unwrap_or_else(|e| unsafe {
									rdf_rs::Datatype::new_unchecked(e.into_iri())
								});
							Some(CompoundLiteral {
								value: Value::Literal(vocabulary.insert_owned_literal(
									Literal::new(string.to_string(), rdf_rs::LiteralType::Any(ty)),
								)),
								triples: None,
							})
						}
						...
						None => match language {
							Some(tag) => Some(CompoundLiteral {
								value: Value::Literal(vocabulary.insert_owned_literal(
									Literal::new(
										string.to_string(),
										rdf_rs::LiteralType::LangString(tag),
									),
								)),
								triples: None,
							}),
							None => Some(CompoundLiteral {
								value: Value::Literal(vocabulary.insert_owned_literal(
									Literal::new(
										string.to_string(),
										rdf_rs::LiteralType::Any(static_datatype(XSD_STRING)),
									),
								)),
								triples: None,
							}),
			...
			Self::Literal(lit, ty) => {
					...
					value::Literal::Number(n) => {
								...
								.unwrap_or(false)
						{
							...
							(
								...
								Some(static_datatype(XSD_DOUBLE)),
							)
						}
					}
				...
				Some(CompoundLiteral {
					value: Value::Literal(vocabulary.insert_owned_literal(Literal::new(
						rdf_lit,
						rdf_rs::LiteralType::Any(
							rdf_ty.unwrap_or_else(|| static_datatype(XSD_STRING)),
						),
					))),
					triples: None,
				})
...
impl<'a, T, B> NestedListTriples<'a, T, B> {
	...
	fn next<V: Vocabulary<Iri = T, BlankId = B>, G: LocalGenerator>(
		&mut self,
		vocabulary: &mut V,
		generator: &mut G,
...
impl<'a, T, B, L> CompoundValueTriples<'a, T, B, L> {
	...
	pub fn next<V, G: LocalGenerator>(
		&mut self,
		vocabulary: &mut V,
		generator: &mut G,
		rdf_direction: Option<RdfDirection>,
...
impl<'a, 'n, N: Vocabulary + VocabularyMut, G: LocalGenerator> Iterator
