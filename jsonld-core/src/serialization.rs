...
use crate::{
    ...
    Value,
    object::{List, Literal},
    rdf::{RDF_FIRST, RDF_JSON, RDF_NIL, RDF_REST, RDF_TYPE, XSD_BOOLEAN, XSD_DOUBLE, XSD_INTEGER, XSD_STRING},
};
...
impl<R> SerResource<R> {
    fn is_empty(&self) -> bool {
        self.types.is_empty() && self.properties.is_empty() && self.graph.is_none() && self.list.is_empty()
    }
...
impl<I, B> ExpandedDocument<I, B> {
    pub fn from_interpreted_quads_in<'a, V, T>(
        vocabulary: &V,
        interpretation: &T,
        quads: impl IntoIterator<Item = Quad<&'a T::Resource, &'a T::Resource, &'a T::Resource, &'a T::Resource>>,
        ...
        B: Clone + Eq + Hash,
    {
        ...
        for (id, resource) in &graph.resources {
            if resource.references != 1 && !resource.is_empty() {
                result.insert(render_object(vocabulary, interpretation, rdf_terms, &graph, id, resource, context)?);
            }
        }
    ...
    where
        ...
        B: Clone + Eq + Hash,
    {
        Self::from_interpreted_quads_in(vocabulary, interpretation, quads, ld_core::Context::default())
    }
}
    ...
    I::Resource: Ord,
{
    match graph.get(&id) {
        Some(resource) => {
            if resource.references == 1 && !resource.is_empty() {
                render_object(vocabulary, interpretation, rdf_terms, graph, id, resource, context)
            } else {
                render_reference(vocabulary, interpretation, id, context)
            }
        }
    ...
    T::BlankId: Clone,
{
    interpretation
        ...
        .or_else(|| interpretation.blank_ids_of(resource).next().map(|b| Id::Valid(ValidId::Blank(b.clone()))))
}

type ResourceTerm<V> = SerTerm<Id<<V as IriVocabulary>::Iri, <V as BlankIdVocabulary>::BlankId>, Value<<V as IriVocabulary>::Iri>>;
