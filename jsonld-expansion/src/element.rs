use crate::{
	Error, Expanded, GivenLiteralValue, LiteralValue, Loader, Options, Warning, WarningHandler,
	expand_array, expand_iri, expand_literal, expand_node, expand_value,
};
...
impl<'a> ActiveProperty<'a> {
	...
	) -> Option<jsonld_core::context::TermDefinitionRef<'c, T, B>> {
		match self {
	...
	match element {
		...
		Value::Object(element) => {
				...
				.map_err(Error::duplicate_key_ref)?
			{
				...
				let local_context =
					jsonld_syntax::context::Context::try_from_json(local_context.clone())?;
