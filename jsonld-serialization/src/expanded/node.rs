use iref::Iri;
use jsonld_core::{object::node::Multiset, Indexed, Node, Object};
...
impl<'a, I, V: Vocabulary> SerializeNode<'a, I, V> {
	pub fn new(
		vocabulary: &'a mut V,
		interpretation: &'a mut I,
		id: Option<jsonld_core::Id<V::Iri, V::BlankId>>,
	) -> Self {
		let result = match id {
...
) -> Result<jsonld_core::Id<I, B>, Indexed<Object<I, B>>> {
	match obj.index() {
		Some(_) => Err(obj),
...
pub(crate) fn is_iri<V, B>(vocabulary: &V, id: &jsonld_core::Id<V::Iri, B>, iri: &Iri) -> bool
where
	V: IriVocabulary,
{
	match id {
		jsonld_core::Id::Valid(rdf_types::Id::Iri(i)) => match vocabulary.iri(i) {
			Some(i) => i == iri,
			None => false,
		},
		_ => false,
