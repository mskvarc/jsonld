...
fn is_static_iri_path(p: &syn::TypePath) -> bool {
    ...
    let head_ok = match p.path.segments.len() {
        ...
        2 => {
            ...
            head.arguments.is_empty() && (head.ident == "iri_rs" || head.ident == "iref" || head.ident == "jsonld")
        }
        _ => false,
    };
...
fn is_bool_path(p: &syn::TypePath) -> bool {
    p.qself.is_none()
        && ((p.path.segments.len() == 3
            ...
            || (p.path.leading_colon.is_none() && p.path.segments.len() == 1 && segment_is_empty_ident(&p.path, 0, "bool")))
}

fn is_str_path(p: &syn::TypePath) -> bool {
    p.qself.is_none()
        && ((p.path.segments.len() == 2
            && (segment_is_empty_ident(&p.path, 0, "std") || segment_is_empty_ident(&p.path, 0, "core") || segment_is_empty_ident(&p.path, 0, "alloc"))
            && segment_is_empty_ident(&p.path, 1, "str"))
            || (p.path.leading_colon.is_none() && p.path.segments.len() == 1 && segment_is_empty_ident(&p.path, 0, "str")))
}

fn is_iri_path(p: &syn::TypePath) -> bool {
    p.qself.is_none()
        && ((p.path.segments.len() == 2 && segment_is_empty_ident(&p.path, 0, "iref") && segment_is_ident(&p.path, 1, "Iri"))
            || (p.path.leading_colon.is_none() && p.path.segments.len() == 1 && segment_is_ident(&p.path, 0, "Iri")))
}

fn is_processing_mode_path(p: &syn::TypePath) -> bool {
    p.qself.is_none()
        && ((p.path.segments.len() == 2 && segment_is_empty_ident(&p.path, 0, "jsonld") && segment_is_empty_ident(&p.path, 1, "ProcessingMode"))
            || (p.path.leading_colon.is_none() && p.path.segments.len() == 1 && segment_is_empty_ident(&p.path, 0, "ProcessingMode")))
}

fn is_rdf_direction_path(p: &syn::TypePath) -> bool {
    p.qself.is_none()
        && ((p.path.segments.len() == 2 && segment_is_empty_ident(&p.path, 0, "jsonld") && segment_is_empty_ident(&p.path, 1, "RdfDirection"))
            || (p.path.leading_colon.is_none() && p.path.segments.len() == 1 && segment_is_empty_ident(&p.path, 0, "RdfDirection")))
}

fn is_option_path(p: &syn::TypePath) -> bool {
    p.qself.is_none()
        && ((p.path.segments.len() == 3
            && (segment_is_empty_ident(&p.path, 0, "std") || segment_is_empty_ident(&p.path, 0, "core"))
            && segment_is_empty_ident(&p.path, 1, "option")
            && segment_is_ident(&p.path, 2, "Option"))
            || (p.path.leading_colon.is_none() && p.path.segments.len() == 1 && segment_is_ident(&p.path, 0, "Option")))
        && is_option_arguments(&p.path.segments.last().unwrap().arguments)
}

fn is_option_arguments(args: &syn::PathArguments) -> bool {
    match args {
        syn::PathArguments::AngleBracketed(args) => args.args.len() == 1 && matches!(&args.args[0], syn::GenericArgument::Type(_)),
        _ => false,
    }
}
...
fn path_into_option(p: syn::TypePath) -> Result<syn::Type, syn::TypePath> {
    if is_option_path(&p) {
        match p.path.segments.into_iter().next_back().unwrap().arguments {
            syn::PathArguments::AngleBracketed(args) => match args.args.into_iter().next_back().unwrap() {
