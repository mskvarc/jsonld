...
fn property_scoped() -> Scenario {
	let context = r#"{"@vocab":"http://ex.org/","child":{"@id":"http://ex.org/child","@context":{"name":"http://schema.org/name","age":"http://schema.org/age"}}}"#;
	let doc = format!(
		r#"{{"@context":{ctx},"child":{{"name":"Bob","age":12}}}}"#,
		ctx = context
	);
...
fn protected_terms() -> Scenario {
	...
	let doc = format!(
		r#"{{"@context":{ctx},"name":"Alice","age":30}}"#,
		ctx = context
	);
...
fn nested_keyword() -> Scenario {
	let context = r#"{"@vocab":"http://ex.org/","details":"@nest","name":"http://schema.org/name","age":"http://schema.org/age","email":"http://schema.org/email"}"#;
	let doc = format!(
...
fn graph_named() -> Scenario {
	let context = r#"{"@vocab":"http://schema.org/"}"#;
	let mut doc = format!(
		r#"{{"@context":{ctx},"@id":"https://ex.org/g1","@graph":["#,
		ctx = context
	);
	for i in 0..50 {
		if i > 0 {
		...
		doc.push_str(&format!(
			r#"{{"@id":"https://ex.org/p{i}","name":"P{i}","age":{}}}"#,
			20 + i
		));
	}
	doc.push_str("]}");
...
fn blank_nodes_many(n: usize) -> Scenario {
	...
	for i in 0..n {
		...
		let next = (i + 1) % n;
		doc.push_str(&format!(
			r#"{{"@id":"_:n{i}","name":"Node{i}","knows":"_:n{next}"}}"#
		));
	}
	doc.push_str("]}");
...
fn mixed_realistic() -> Scenario {
	...
	for i in 0..20 {
		...
		doc.push_str(&format!(
			r#"{{"@id":"https://ex.org/c{i}","name":{{"en":"Colleague {i}"}}}}"#
		));
	}
	doc.push_str("]}");
...
fn index_map_large(n: usize) -> Scenario {
	...
	for i in 0..n {
		...
		doc.push_str(&format!(
			r#""key-{i}":{{"@id":"https://ex.org/i{i}","name":"E{i}","val":{i}}}"#
		));
	}
	doc.push_str("}}");
...
fn array_of_value_objects(n: usize) -> Scenario {
	...
	for i in 0..n {
		...
		doc.push_str(&format!(
			r#"{{"@value":"v{i}","@language":"en","@direction":"ltr"}}"#
		));
	}
	doc.push_str("]}");
