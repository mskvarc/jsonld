...
impl IndexTerm {
    fn from_local_term(t: LocalTerm, vocabulary: &mut IndexVocabulary) -> Self {
        match t {
            ...
            LocalTerm::Named(RdfTerm::Literal(lit)) => Self::Literal(vocabulary.insert_owned_literal(lit)),
    ...
    async fn async_run(self) {
        ...
        match self.desc {
            to_rdf::Description::Positive { expect } => {
                ...
                let expected_content = std::fs::read_to_string(loader.filepath(expect).unwrap()).unwrap();
                let (parsed, _code_map) = n_quads::grdf_document_from_str(&expected_content).unwrap();
